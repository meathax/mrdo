...
//
// The CRTC runs at one character clock per eight pixels.  Unlike the earlier
// fixed-raster approximation, vertical total, total-adjust, maximum raster,
// displayed rows, sync position, start address, MA/RA and CURSOR all come from
// the programmed registers.  The default register image is the measured
// 312x264, 240x192 board mode, so reset never emits a malformed partial frame.
//============================================================================
module docastle_crtc
(
	input clk, input reset, input ce_pix, input cursor_irq_mode,
	input [4:0] reg_sel, input [7:0] reg_data, input reg_we,
	output [8:0] h_count, output [8:0] v_count,
	output hs, output vs, output hblank, output vblank, output cursor,
	output [13:0] ma, output [4:0] ra, output main_irq_n,
	output reg sub_irq_req, output reg sprite_nmi_req
);
...
assign hblank = (h_ctr < 9'd8) || ({1'b0,h_ctr} >= h_active_end);
assign vblank = in_adjust || (row_ctr >= crtc[6][6:0]);
